<div class="bg-gradient-to-br from-gray-100 to-indigo-100 font-sans min-h-screen">
    <div class="container-fluid px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white/90 shadow-2xl rounded-3xl p-10 glassmorphic tilt slide-up border border-indigo-200/50"
            x-data="editMockOneQuestionManager" x-init="init">
            <h1
                    ...
                    <ul class="max-h-60 overflow-y-auto">
                        <template x-for="section in sectionSearchResults" :key="section.unique_id">
                            <li @click="selectSection(section.unique_id, section.tag, section.name)"
                                ...
                                <span x-text="'Section ' + section.tag + ' - ' + section.name + ' (ID: ' + section.unique_id + ') - Mock Test-1'"></span>
                            </li>
                        </template>
                    </ul>
                </div>
                    ...
                    <template x-for="(field, index) in questionFields" :key="field.unique_id">
                        <div class="p-4 bg-white/80 rounded-xl shadow-md border border-gray-200">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                                ...
                                <div>
                                    <span class="font-semibold text-purple-900">D:</span>
                                    <span x-text="field.option_d"></span>
                                </div>
                                <div>
                                    <span class="font-semibold text-purple-900">E:</span>
                                    <span x-text="field.option_e"></span>
                                </div>
                                <div>
                                    <span class="font-semibold text-purple-900">F:</span>
                                    <span x-text="field.option_f"></span>
                                </div>
                            </div>
                            <div class="mt-2">
                                <span class="font-semibold text-purple-900">Answer(s):</span>
                                <span
                                    x-text="[field.answer_1, field.answer_2, field.answer_3].filter(Boolean).join(', ')"></span>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div x-show="showEditModal"
                ...
                <div class="bg-white rounded-xl p-6 w-full max-w-2xl">
                    ...
                    <div class="space-y-4">
                        ...
                        <template x-for="option in ['a', 'b', 'c', 'd', 'e', 'f']" :key="option">
                            <div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 gap-4">
                                <template x-for="opt in ['a', 'b', 'c', 'd', 'e', 'f']" :key="opt">
                            ...
                            <div x-show="editForm.answers.length !== 3" class="mt-2 text-red-600">
                                Please select exactly three answers.
                            </div>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end space-x-4">
                        ...
                        <button @click="updateQuestion()" :disabled="isSubmitting || editForm.answers.length !== 3"
                            class="px-4 py-2 text-white bg-purple-900 hover:bg-purple-700 rounded-xl flex items-center"
                            :class="{ 'bg-gray-400 cursor-not-allowed': isSubmitting || editForm.answers.length !== 3 }">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('editMockOneQuestionManager', () => ({
            sectionSearch: '',
            ...
            editIndex: null,
            init() {
                console.log('EditMockOneQuestionManager initialized');
            },
            async searchSections() {
                ...
                const url = '{{ route('editmockonequestion.search') }}?query=' + encodeURIComponent(query);
            ...
            async selectSection(unique_id, tag, name) {
                ...
                this.selectedSection = `Section ${tag} - ${name} (ID: ${unique_id}) - Mock Test-1`;
                this.sectionSearch = '';
                this.sectionSearchResults = [];
                this.questionFields = [];
            ...
            async loadQuestions() {
                ...
                this.isLoading = true;
                const url = '{{ route('editmockonequestion.questions') }}?section_unique_id=' +
                ...
                try {
                    ...
                    this.questionFields = questions.map(q => ({
                        ...
                        option_e: q.option_e,
                        option_f: q.option_f,
                        answers: [q.answer_1, q.answer_2, q.answer_3].filter(Boolean).sort(),
                        answer_1: q.answer_1 || '',
                        answer_2: q.answer_2 || '',
                        answer_3: q.answer_3 || ''
                    }));
            ...
            updateEditAnswers(value) {
                ...
                } else if (this.editForm.answers.length < 3) {
                ...
                this.editForm.answer_3 = this.editForm.answers[2] || '';
                console.log('Updated edit answers:', this.editForm.answers);
            },
            async updateQuestion() {
                if (this.editForm.answers.length !== 3) {
                    this.formStatus = 'error';
                    this.formMessage = 'Please select exactly three answers.';
                    setTimeout(() => {
                        this.formMessage = '';
                        this.formStatus = '';
                    }, 3000);
                ...
                const url = '{{ route('editmockonequestion.update') }}';
            ...
            async confirmDelete() {
                this.isSubmitting = true;
                const url = '{{ route('editmockonequestion.delete') }}';
            ...
            async deleteAllQuestions() {
                ...
                const url = '{{ route('editmockonequestion.delete-all') }}';
