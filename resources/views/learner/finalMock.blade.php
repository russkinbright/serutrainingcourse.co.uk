...
@section('content')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-white rounded-xl shadow-md p-4 flex flex-col text-purple-800">
                        ...
                        <div class="text-center justify-center">
                            <button @click="initFinalMock()" {{-- @click="window.location.reload()" --}}
        ...
        <template x-if="quizContent && !quizContent.resultFeedback">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="bg-purple-50 p-4 rounded-lg">
                    <!-- Progress Indicator -->
                    <div class="card bg-purple-700 text-center mb-4 text-white font-bold rounded py-3">
                        Page <span x-text="currentPage + 1"></span> of <span x-text="totalPages"></span>
                        (Questions <span x-text="(currentPage * questionsPerPage) + 1"></span> -
                        <span x-text="Math.min((currentPage + 1) * questionsPerPage, quizContent.questions.length)"></span>)
                    </div>

                    <template
                        x-if="quizContent.questions && Array.isArray(quizContent.questions) && quizContent.questions.length > 0">
                        <template
                            x-for="(q, qIndex) in quizContent.questions.slice(currentPage * questionsPerPage, (currentPage + 1) * questionsPerPage)"
                                ...
                                <div class="flex justify-between items-center mb-3">
                                    <div class="flex gap-2">
                                        <p class="font-bold text-lg text-purple-600"
                                            x-html="`Q${(currentPage * questionsPerPage) + qIndex + 1}:`"></p>
                                        <span class="text font-bold space-x-2 space-y-4"
                                            x-html="formatQuestionText(q, (currentPage * questionsPerPage) + qIndex)"></span>
                                    </div>
                                </div>
                                    ...
                                    <template x-if="q.type === 'single'">
                                        <template x-for="option in ['a', 'b', 'c']" :key="option">
                                            <label
                                                ...
                                                <input type="radio"
                                                    :name="`q_${(currentPage * questionsPerPage) + qIndex}`"
                                                    :value="option"
                                                    :checked="quizContent.selectedAnswers[(currentPage * questionsPerPage) +
                                                        qIndex] === option"
                                                    @change="quizContent.selectedAnswers[(currentPage * questionsPerPage) + qIndex] = option"
                                        ...
                                        <template x-for="option in ['a', 'b', 'c']" :key="option">
                                            <label
                                                ...
                                                <input type="checkbox"
                                                    ...
                                                    :value="option"
                                                    :checked="quizContent.selectedAnswers[(currentPage * questionsPerPage) +
                                                        qIndex]?.includes(option)"
                                                    @change="updateDoubleAnswer((currentPage * questionsPerPage) + qIndex, option)"
                    ...
                    <div class="flex justify-center mt-6 gap-5">
                        <button @click="previousPage()"
                            class="w-96 bg-rose-500 cursor-pointer hover:bg-pink-400 text-white text-xl font-bold py-4 px-4 rounded-xl shadow-lg border-2 border-white 
                        ...
                        <template x-if="currentPage < totalPages - 1">
                            <button @click="nextPage()"
                                class="w-96 bg-purple-600 cursor-pointer hover:bg-purple-700 text-white text-xl font-bold py-1.5 px-4 rounded-xl shadow-lg border-2 border-white 
                                hover:shadow-xl hover:scale-95 transition-all duration-300 button">
                                <span>Next</span>
                            </button>
                        </template>
                        <template x-if="currentPage === totalPages - 1">
                            <button @click="checkAnswersLocally(quizContent)"
                                class="w-1/3 bg-purple-600 cursor-pointer hover:bg-purple-700 text-white font-semibold py-1.5 px-4 rounded-xl shadow-lg border-2 border-white 
                                hover:shadow-xl hover:scale-95 transition-all duration-300 button">
    ...
    <script>
        function finalMockApp() {
            return {
                ...
                initFinalMock() {
                    this.currentPage = 0;
                    const preload = window.__FINAL_MOCK__?.questions || [];
                    if (preload.length > 0) {
                        this.quizContent = {
                            type: 'quiz',
                            title: 'Final Mock Test',
                            questions: preload,
                            selectedAnswers: preload.some(q => q.type === 'mock1') ?
                                Array(preload.length).fill([]).map((_, i) => Array(this.getBlanksCount(preload[i]))
                                    .fill('')) : Array(preload.length).fill(''),
                            resultFeedback: null
                        };
                        this.totalPages = Math.ceil(this.quizContent.questions.length / this.questionsPerPage);
                    } else {
                        // fallback to API if user somehow hits the page directly
                    ...
                    try {
                        ...
                        if (!res.ok) {
                            ...
                            const message = res.status === 422 ?
                                'Not enough questions available to create the final mock test. Please contact support.' :
                                errorData.message || `Failed to load questions (HTTP ${res.status})`;
                            ...
                            this.quizContent = {
                                ...
                                selectedAnswers: data.questions.some(q => q.type === 'mock1') ?
                                    Array(data.questions.length).fill([]).map((_, i) => Array(this.getBlanksCount(
                                        data.questions[i])).fill('')) : Array(data.questions.length).fill(''),
                                resultFeedback: null
                            };
                            this.totalPages = Math.ceil(this.quizContent.questions.length / this.questionsPerPage);
                        } else {
    ...
    <script>
        window.__FINAL_MOCK__ = {
            questions: @json(session('final_mock_questions', [])),
        };
    </script>
@endsection
