    ...
    <div class="mx-auto px-4 py-6 flex flex-col lg:flex-row gap-6" x-data="continueCourseApp()" x-init="initContinueCourse('{{ $unique_id }}')">
        ...
        <div class="lg:w-1/3 bg-white rounded-2xl shadow-lg p-6 resize-panel" id="leftPanel" x-show="!isLeftPanelHidden"
            ...
            <div class="sticky top-4 space-y-4">
                <!-- Course Header -->
                <div class="mb-6 bg-gradient-to-r from-purple-800 to-purple-600 text-white shadow-xl rounded-2xl p-8 transform transition hover:shadow-2xl">
                    <h1 class="text-xl font-extrabold tracking-tight" x-text="selectedCourse?.title || 'Loading Course...'"></h1>
                </div>
                        ...
                        <div x-data="{ open: false }" class="border border-purple-200 rounded-lg mb-3 overflow-hidden">
                            ...
                            <div x-show="open" class="px-4 py-3 bg-white border-t border-purple-200">
                                <ul class="space-y-2">
                                    ...
                                    <template x-if="section.quizzes && Array.isArray(section.quizzes) && section.quizzes.length > 0">
        ...
        <div class="lg:flex-1 bg-white rounded-2xl shadow-lg p-6" :class="{ 'lg:w-full': isLeftPanelHidden }">
            ...
            <template x-if="selectedContent?.resultFeedback">
                <div class="mt-8 p-6 bg-gradient-to-br from-purple-100 to-purple-50 border border-purple-200 rounded-2xl shadow-xl">
                        ...
                        <div class="bg-white rounded-xl shadow-md p-4 flex flex-col text-purple-800">
                            ...
                            <div class="mt-3 text-center">
                                <template x-if="isQuizPassed(selectedContent.resultFeedback, selectedContent.questions)">
                                    <div class="text-green-600 font-semibold">🎉 Congratulations, you passed the quiz!</div>
                                </template>
                                <template x-if="!isQuizPassed(selectedContent.resultFeedback, selectedContent.questions)">
                            ...
                            <template x-if="selectedContent.quiz_type === 'mock1' && getIncorrectMock1Feedback().length > 0">
                    ...
                    <div class="bg-purple-50 p-4 rounded-lg">
                        <template x-if="selectedContent.type === 'section'">
                            <div class="text-gray-500 italic">
                                This section provides access to quizzes. Select a quiz from the list to begin.
                            </div>
                        </template>
                        <template x-if="selectedContent.type === 'quiz'">
                            <div>
                                <template x-if="selectedContent.questions && Array.isArray(selectedContent.questions) && selectedContent.questions.length > 0">
                                    <template x-for="(q, qIndex) in selectedContent.questions" :key="q.unique_id || qIndex">
                                        <div class="mb-6 p-4 bg-white rounded-lg shadow-sm border border-purple-200">
                                            <div class="flex justify-between items-center mb-3">
                                                <div class="flex gap-2">
                                                    ...
                                                    <span class="text font-bold space-x-2 space-y-4" x-html="`${formatQuestionText(q, qIndex)}`"></span>
                                                </div>
                                                <template x-if="selectedContent.resultFeedback">
                                                    <span>
                                            ...
                                            <template x-if="selectedContent.quiz_type === 'practice' || selectedContent.quiz_type === 'mock2'">
                                                <div class="space-y-2">
                                                    <template x-if="q.type === 'single'">
                                                        <template x-for="option in ['a', 'b', 'c']" :key="option">
                                                            <label class="flex items-center space-x-2 cursor-pointer p-2 rounded transition-colors duration-200"
                                                                :class="{
                                                                    'border border-green-400 bg-green-100 text-green-800 font-semibold': selectedContent.resultFeedback && selectedContent.resultFeedback[qIndex]?.isCorrect && selectedContent.selectedAnswers[qIndex] === option,
                                                                    'border border-red-400 bg-red-100 text-red-800 font-semibold': selectedContent.resultFeedback && !selectedContent.resultFeedback[qIndex]?.isCorrect && selectedContent.selectedAnswers[qIndex] === option
                                                                }">
                                                                ...
                                                                <span x-text="option.toUpperCase() + '. ' + (q['option_' + option] || '')"></span>
                                                            </label>
                                                        </template>
                                                    </template>
                                                    <template x-if="q.type === 'double'">
                                                        <template x-for="option in ['a', 'b', 'c']" :key="option">
                                                            <label class="flex items-center space-x-2 cursor-pointer p-2 rounded transition-colors duration-200"
                                                                :class="{
                                                                    'border border-green-400 bg-green-100 text-green-800 font-semibold': selectedContent.resultFeedback && selectedContent.resultFeedback[qIndex]?.selected.includes(option) && selectedContent.resultFeedback[qIndex]?.isCorrect,
                                                                    'border border-red-400 bg-red-100 text-red-800 font-semibold': selectedContent.resultFeedback && selectedContent.resultFeedback[qIndex]?.selected.includes(option) && !selectedContent.resultFeedback[qIndex]?.isCorrect
                                                                }">
                                                                <input type="checkbox" :name="`q_${qIndex}`"
                                                                    :value="option"
                                                                    :checked="selectedContent.selectedAnswers[qIndex]?.includes(option)"
                                                    ...
                                                    <template x-if="selectedContent.resultFeedback && !selectedContent.resultFeedback[qIndex]?.isCorrect">
                                                        <div class="mt-4 flex items-start space-x-2">
                                                            <span class="font-bold bg-pink-600 rounded-lg p-2 text-white text-sm">Hints:</span>
                                                            <div class="text-rose-600 font-bold mt-1" x-text="q.incorrect"></div>
                                                        </div>
                                                    </template>
                                                </div>
                                            </template>     
                                ...
                                <template x-if="selectedContent.quiz_type && (!selectedContent.questions || !Array.isArray(selectedContent.questions) || selectedContent.questions.length === 0)">
                                    <div class="text-gray-500 italic flex items-center justify-center h-full">
        ...
        function continueCourseApp() {
            return {
                ...
                startQuiz() {
                    ...
                    if (!this.selectedQuizRaw || !this.selectedQuizRaw.questions || !Array.isArray(this.selectedQuizRaw.questions) || this.selectedQuizRaw.questions.length === 0) {
                    ...
                    this.selectedContent = {
                        ...
                        selectedAnswers: this.selectedQuizType === 'mock1' ? Array(this.selectedQuizRaw.questions.length).fill([]).map((_, i) => Array(this.getBlanksCount(this.selectedQuizRaw.questions[i])).fill('')) : {}
                    };
                ...
                updateBlankAnswer(qIndex, blankIndex, value) {
                    ...
                    if (!this.selectedContent.selectedAnswers[qIndex]) {
                        this.selectedContent.selectedAnswers[qIndex] = Array(this.getBlanksCount(this.selectedContent.questions[qIndex])).fill('');
                    }
                ...
                getIncorrectMock1Feedback() {
                    ...
                    this.selectedContent.resultFeedback.forEach((feedback, qIndex) => {
                        if (!feedback.isCorrect) {
                            ...
                            const correctAnswers = [question.answer_1, question.answer_2, question.answer_3].filter(Boolean);
                            ...
                            selectedText = `<br><span class="text-purple-600 font-bold">You selected:</span> <span class="text-black space-x-3">${selectedText}</span>`;
                            feedbackList.push({
                                qIndex,
                                correctText,
                                selectedText
                ...
                checkAnswersLocally(content) {
                    ...
                    content.resultFeedback = content.questions.map((q, index) => {
                        if (content.quiz_type === 'mock1') {
                            ...
                            const isCorrect = selected.length === correct.length && selected.every((val, i) => val === correct[i]);
                            console.log(`Q${index + 1}: selected=${selected}, correct=${correct}, isCorrect=${isCorrect}`);
                            return {
