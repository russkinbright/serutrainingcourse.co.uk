...
class StripeController extends Controller
{
    public function payment(Request $request)
    {
        try {
            ...
            // If Stripe redirected back with a session_id, we are in Phase B.
            if ($request->has('session_id')) {
                $sessionId = $request->query('session_id');

                // Pull cart/billing you stored earlier
                ...
                // Verify the session & that payment is completed
                ...
                // Idempotency: if already processed, just show success
                if ($paymentIntentId && Payment::where('transaction_id', $paymentIntentId)->exists()) {
                    return view('emails.successPayment');
                }

                // Recompute total from cart & validate courses (like your PayPal flow)
                $calcTotal = 0;
                ...
                // Create or fetch Learner (same as PayPal)
                ...
                // Payments + CourseProgress (same as PayPal)
                foreach ($cartItems as $item) {
                    $course = Course::where('unique_id', $item['unique_id'])->first();
                    if (!$course) {
                        Log::error('Course not found for unique_id: '.$item['unique_id']);
            ...
            // ---------- Phase A: first time here -> create Checkout session and redirect ----------
            // Validate & recompute total like above (optional but safer)
            $cartItems = Session::get('stripe_cart', []);
            $billing   = Session::get('stripe_billing', []);
            $total     = (float) Session::get('stripe_total', 0);
            ...
            // Create the Checkout session and send the user away
            $checkoutSession = StripeSession::create([
                ...
                'mode'        => 'payment',
                // IMPORTANT: return to the SAME controller method with session_id
                'success_url' => route('stripe.payment') . '?session_id={CHECKOUT_SESSION_ID}',
